...
import { CATEGORIES, getPhaseVocabulary } from './content';
...
// ─── Daily Challenge ──────────────────────────────────────────────────────────
export async function saveDailyChallenge(score, userId) {
  const progress = await loadProgress(userId);
  progress.dailyChallenge = { lastDate: todayStr(), score };
  addXP(progress, score * 20);
  await saveProgress(progress, userId);
  return progress;
}

// ─── Redeem ───────────────────────────────────────────────────────────────────
export function getLearnedWords(progress) {
  const learned = [];
  for (const [phaseStr, slot] of Object.entries(progress.phases || {})) {
    const ph = parseInt(phaseStr);
    const vocab = getPhaseVocabulary(ph);
    for (const catId of Object.keys(slot.flashcards || {})) {
      const indices = slot.flashcards[catId] || [];
      const words = vocab[catId] || [];
      for (const idx of indices) {
        if (words[idx]) learned.push(words[idx]);
      }
    }
  }
  return learned;
...
export async function redeemGift(giftId, giftName, arabicName, xpCost, userId) {
  const progress = await loadProgress(userId);
  progress.totalXP = Math.max(0, (progress.totalXP ?? 0) - xpCost);
  await saveProgress(progress, userId);
  if (userId) {
  ...
  try {
    fetch('/api/notify-redemption', {
      ...
      body: JSON.stringify({ giftName, arabicName, xpCost, userId }),
    });
  } catch (_) {}
  return progress;
}
